...
read-only interface for terminal prepared and standalone Coordinator Runs.
...
semantically correct, or that it has been accepted. Missing records, unavailable
local Git objects, supported legacy evidence without the newer proof, and
oversized private proof produce `"unavailable"` with a bounded reason and the
affected identity. Callers that continue execution must reject unavailable
proof. Malformed records, contradictory subjects or digests, cycles, invalid
continuations, and unsafe paths raise `RunValidationError`. Publication's
existing legacy adapters may still produce their older partial schemas, but do
not turn unavailable private proof into verified authority.
