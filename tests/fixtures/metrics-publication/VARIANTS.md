...
| Original and continuations | A source invocation identity can occur in several selected Runs. Uniqueness is within each Run. Stage ownership always uses the exact selected Run ID. Two retained paths count once only when their authenticated invocation identity is the same; a distinct sealed receipt is invalid. | `evidence-coverage-variants.json` shared-continuation case; `test_each_exhausted_continuation_adds_a_fresh_response_allowance`; `test_continuation_alias_rejects_a_distinct_authenticated_receipt` |
...
`populated/evidence-coverage-variants.json` is the regenerated portable companion matrix for the two zero/deduplicated coverage cases whose absence cannot be represented by adding invocation rows to a valid publication. `populated/invalid-mutations.json` describes independent changes to a fresh copy of the populated valid publication. Each must fail consumer intake. Paths use JSON object keys and array indices; `-1` means the last array row. Mutations cover Pi discriminator/field disagreement, minimal Pi cost, invocation provenance on totals, unknown token categories, invalid Run purpose, and Pi fields on abandoned evidence. They are test inputs, not a second validator or a schema change.
