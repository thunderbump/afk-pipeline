...
def main():
    ...
    while state["status"] == "running":
        ...
        if active is None:
            ...
            component_input, input_from = COMPONENTS[component]["build_input"](
...
def validate_terminal(status, terminal):
    ...
    if (
        ...
        or terminal["failed_component"] not in COMPONENTS
...
def validate_terminal_history(state):
    ...
    if (
        ...
        or last["outcome"] == COMPONENTS[last["component"]]["success"]
    ):
        raise ValueError("invalid coordinator checkpoint")
...
def validate_history_position(state):
    ...
    for expected_sequence, record in enumerate(state["history"], start=1):
        ...
        if record["outcome"] == "abandoned":
            continue
        if record["outcome"] != COMPONENTS[expected_component]["success"]:
            expected_component = None
        else:
            expected_component = COMPONENTS[expected_component]["next"]
...
def latest_record(history, *components):
    for record in reversed(history):
        if (
            ...
            and record["outcome"] == COMPONENTS[record["component"]]["success"]
        ):
            return record
    raise ValueError("invalid coordinator checkpoint")
...
def attempt_input(_request, assignment, _state, _run_directory):
    return assignment, {"assignment": "assignment.json"}


def validation_input(request, assignment, state, _run_directory):
    source = latest(state, "attempt", "response")["directory"]
    return {
        "schema_version": 1,
        "workspace": assignment["workspace"],
        **request["validation"],
    }, {"workspace": "assignment.json", "change": source}
